...
const BalancePage = () => {
  ...
  const handleReceiveMoney = () => {
    ...
    // No need to navigate away after clicking receive, remove this line:
  ...
  const handleSendMoney = () => {
    ...
    // No need to navigate away after clicking send, remove this line:
            ...
            <li key={index} className="transaction-item">
              <div className="transaction-details">
                <div className="transaction-icon">
                  {transaction.type === "receive" ? "R" : "S"}
                </div>
                <div className="transaction-info">
                  <span className="transaction-name">
                    {transaction.type === "receive" ? "Received" : "Sent"}
                  </span>
                  {/* For time, you might want to format it properly */}
                  <span className="transaction-time">
                    {new Date().toLocaleTimeString()} - {transaction.type}
                  </span>
                </div>
                <span className="transaction-amount">
                  {transaction.type === "receive" ? `+$${transaction.amount.toFixed(2)}` : `-$${transaction.amount.toFixed(2)}`}
                </span>
              </div>
            </li>
          ))}
